...
class TypeCheckerTest extends Specification {
    ...
    def "TypeChecker should check type and map correctly"() {
        ...
        def length = TypeChecker.whenTypeOf("123").is(String.class)
                .thenReturn({s -> s.length()}).execute()
    ...
    def "TypeChecker should chose first match"() {
        ...
        def v = TypeChecker.whenTypeOf("123")
    ...
    def "TypeChecker execute operations lazy"() {
        ...
        def oper = TypeChecker.whenTypeOf("123").is(Integer.class)
                .thenReturn({i -> i + 1}).is(StringBuffer.class).thenReturn({s -> s.length()})
    ...
    def "TypeChecker should match parent class types"() {
        ...
        def className = TypeChecker.whenTypeOf(new B())
                .is(A.class).thenReturn({c -> "A"}).execute()
